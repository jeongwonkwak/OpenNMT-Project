...
def preprocess_opts(parser):
    ...
    group.add('--train_src', '-train_src', default=["data/src-train-mecab+sp.txt"], nargs='+', #required=True
              help="Path(s) to the training source data")
    group.add('--train_tgt', '-train_tgt', default=["data/tgt-train-mecab+sp.txt"],  nargs='+', #required=True,
    ...
    group.add('--valid_src', '-valid_src', default="data/src-valid-mecab+sp.txt",
              help="Path to the validation source data")
    group.add('--valid_tgt', '-valid_tgt', default="data/tgt-valid-mecab+sp.txt",
    ...
    group.add('--save_data', '-save_data', default="data/dataset_MeSen",# required=True,
...
def train_opts(parser):
    ...
    group.add('--data', '-data', default="data/dataset_MeSen",#required=True,
...
def translate_opts(parser):
    ...
    group.add('--model', '-model', dest='models', metavar='MODEL',
              nargs='+', type=str, default=["data/model//model_token4096_adam_3_lr2_l12.pt"],# required=True,
    ...
    group.add('--src', '-src', default="data/demo/koreanTokenInput.txt", #data/src-test-mecab+sp.txt", #required=True,
              help="Source sequence to decode (one line per "
                   "sequence)")
    group.add('--src_dir', '-src_dir', default="",
              help='Source directory for image or audio files')
    group.add('--tgt', '-tgt',#data/tgt-test-mecab+sp.txt",
    ...
    group.add('--shard_size', '-shard_size', type=int, default=10000,
              help="Divide src and tgt (if applicable) into "
                   ...
                   "shard_size>0 means segment dataset into multiple shards, "
                   "each shard has shard_size samples")
    group.add('--output', '-output', default='data/demo/englishPred.txt', #data/prediction/pred.txt',
    ...
    group.add('--replace_unk', '-replace_unk', action="store_true", default="True",
    ...
    group = parser.add_argument_group('Logging')
    group.add('--verbose', '-verbose', action="store_true", default="True",
